  ...
  _createClass(S3Service, [{
    ...
    value: function getRevisions() {
      ...
      return this.getMetaJson().then(function (json) {
        ...
        return _this.s3AWS.listObjectsAsync({
          Bucket: _this2.bucket,
          Prefix: _this2.revisionFolderPath
        });
      }).then(function (data) {
        var folders = data.Contents;
        return _lodash2.default.chain(folders).orderBy('LastModified', 'desc').map(function (obj) {
          return {
            ID: _lodash2.default.trimEnd(obj.Key.replace(_this2.revisionFolderPath, ''), '/'),
      ...
      return new _bluebird2.default(function (resolve, reject) {
        var params = {
          ...
          s3Params: {
            ...
            Prefix: _path2.default.join(_this3.revisionFolderPath, dest)
          }
        };

        var uploader = _this3.s3.uploadDir(params);
    ...
    value: function addRevisionToJson(revisionHash) {
      ...
      return this.getMetaJson().then(function (json) {
        ...
        return _this4.createMetaJson(json);
      });
    }
  }, {
    key: 'rotate',
    value: function rotate(revisionHash) {
      ...
      .then(function () {
        return _this5.move(lastPath, secondToLastPath);
      }).then(function () {
        return _this5.copy(revisionPath, lastPath);
    ...
    value: function move(src, dest) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        execAsync('aws s3 mv --recursive s3://' + _this7.bucket + '/' + src + ' s3://' + _this7.bucket + '/' + dest).then(function () {
          resolve();
        }).catch(function (err) {
          resolve();
        });
    ...
    value: function copy(src, dest) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        execAsync('aws s3 cp --recursive s3://' + _this8.bucket + '/' + src + ' s3://' + _this8.bucket + '/' + dest).then(function () {
          resolve();
        }).catch(function (err) {
          resolve();
        });
